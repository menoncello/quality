...
export class TriageEngine {
  ...
  private balanceWorkload(suggestions: TriageSuggestion[], _context: ProjectContext): void {
    ...
    suggestions.forEach(suggestion => {
      if (suggestion.assignee && workloadDistribution.has(suggestion.assignee)) {
        ...
        if (currentWorkload && currentWorkload > 10) { // Arbitrary threshold
        ...
        // Update workload
        workloadDistribution.set(suggestion.assignee, (currentWorkload ?? 0) + suggestion.estimatedEffort);
      }
    });
  }
  ...
  private optimizeForDeadlines(suggestions: TriageSuggestion[], context: ProjectContext): void {
    ...
    suggestions.forEach(suggestion => {
      if (suggestion.deadline && context.currentSprint && suggestion.deadline > context.currentSprint.endDate) {
