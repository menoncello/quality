...
export class WorkflowIntegration {
  ...
  private adaptForScrum(
    ...
    adjusted.forEach(prioritization => {
      const goalAlignment = this.calculateGoalAlignment(prioritization, currentSprint);
      ...
      if ((context.currentSprint)) {
        ...
        if (suggestion.action === 'fix-now'  || suggestion.action === 'schedule') {
          const daysUntilSprintEnd = Math.ceil(
            (context.currentSprint?.endDate.getTime() ?? Date.now() - Date.now()) / (1000 * 60 * 60 * 24)
          );
  ...
  private calculateCapacityUtilization(context: ProjectContext): number {
    const currentSprint = context.currentSprint;
    if (!currentSprint) return 0.5;

    return currentSprint.currentLoad / currentSprint.capacity;
  }
