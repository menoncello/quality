...
export class ScoringAlgorithm {
  ...
  private calculateSeverityScore(issue: Issue, classification: IssueClassification): number {
    ...
    const typeSeverity: number = typeSeverityMap[issue.type] || 5;
    ...
    const classificationSeverity: number = classificationSeverityMap[classification.severity] ?? 5;
  ...
  private calculateImpactScore(
    ...
    const criticalityImpact: number = criticalityImpactMap[context.criticality] ?? 0;
  ...
  private calculateEffortScore(
    ...
    const categoryEffort: number = categoryEffortMap[classification.category] ?? 5;
  ...
  private calculateBusinessValueScore(context: IssueContext, projectContext: ProjectContext): number {
    ...
    const criticalityValue: number = criticalityValueMap[context.criticality] ?? 5;
  ...
  private calculateClassificationBonus(classification: IssueClassification): number {
    ...
    const categoryBonus: number = categoryBonuses[classification.category] || 1.0;
    bonus *= categoryBonus;
  ...
  private estimateEffort(issue: Issue, context: IssueContext, classification: IssueClassification): number {
    ...
    const categoryMultiplier: number = categoryMultipliers[classification.category] || 1.0;
