...
import { ResultReporter, ReportFormat } from '../../analysis/result-reporter.js';
...
describe('Result Pipeline Integration Tests', () => {
  ...
  describe('End-to-End Result Processing', () => {
    it('should process complete result pipeline from raw tool outputs to reports', async () => {
      ...
      const reports = await resultReporter.generateMultipleReports(
        ...
        [
          { format: ReportFormat.JSON, includeDetails: true, includeMetrics: true, includeRecommendations: true, includeCharts: false, groupBy: 'tool', sortBy: 'severity' },
          { format: ReportFormat.HTML, includeDetails: true, includeMetrics: true, includeRecommendations: true, includeCharts: false, groupBy: 'tool', sortBy: 'severity' },
          { format: ReportFormat.MARKDOWN, includeDetails: true, includeMetrics: true, includeRecommendations: true, includeCharts: false, groupBy: 'tool', sortBy: 'severity' }
        ]
      );
  ...
  describe('Result Normalization Integration', () => {
    it('should normalize results from different tool types consistently', () => {
      ...
      const normalizedResults = resultNormalizer.normalizeResults(diverseResults as ToolResult[]);
    ...
    it('should handle invalid tool results gracefully', () => {
      ...
      const normalizedResults = resultNormalizer.normalizeResults(invalidResults as ToolResult[]);
    ...
    it('should calculate comprehensive quality scores', () => {
      const mockAggregated = {
        ...
        coverage: {
          ...
          functions: { total: 50, covered: 42, percentage: 85 },
