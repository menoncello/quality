...
describe('ResultAggregator', () => {
  ...
  const createMockResult = (toolName: string, issuesCount: number, errorsCount: number = 0)=> ({
    toolName,
    toolVersion: '1.0.0',
    ...
    it('should generate AI prompts for critical issues', () => {
      ...
      const analysisResult = aggregator.createAnalysisResult(results, {
        ...
        issueStatistics: {
          ...
          bySeverity: {
            errors: results.reduce((sum, r) => sum + (r.metrics.errorsCount ?? 0), 0),
            warnings: results.reduce((sum, r) => sum + (r.metrics.warningsCount ?? 0), 0),
            info: results.reduce((sum, r) => sum + (r.metrics.infoCount ?? 0), 0)
          },
          byCategory: {},
          byTool: {},
          byFile: {},
          fixable: results.reduce((sum, r) => sum + (r.metrics.fixableCount ?? 0), 0),
          critical: results.reduce((sum, r) => sum + (r.issues.filter(i => i.severity === 'error').length), 0),
          duplicatesRemoved: 0
        },
        coverage: null,
        performance: {
