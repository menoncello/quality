...
export class AutoConfigurationDetectionEngine implements DetectionEngine {
  ...
  private generateIssues(
    ...
    // Project type issues
    if ((project as { type?: string }).type === 'unknown') {
      issues.push('Could not determine project type');
    }

    // Tool configuration issues
    const enabledTools = (tools as Array<{ enabled?: boolean; name?: string }>).filter(t => t.enabled);
    if (enabledTools.length === 0) {
      issues.push('No development tools detected');
    }

    // Dependency issues
    if ((compatibility as { issues?: string[] }).issues?.length) {
      issues.push(...(compatibility as { issues: string[] }).issues);
    }

    // Structure issues
    if (!(structure as { sourceDirectories?: string[] }).sourceDirectories?.length) {
      issues.push('No source directories found');
    }

    if (!(structure as { testDirectories?: string[] }).testDirectories?.length) {
      issues.push('No test directories found - consider adding tests');
    }

    // Configuration issues
    const hasLinting = (tools as Array<{ name?: string; enabled?: boolean }>).some(t => t.name === 'eslint' && t.enabled);
    const hasFormatting = (tools as Array<{ name?: string; enabled?: boolean }>).some(t => t.name === 'prettier' && t.enabled);
  ...
  private generateRecommendations(
    ...
    recommendations.push(...(compatibility as { recommendations?: string[] }).recommendations ?? []);

    // Tool recommendations
    const toolNames = (tools as Array<{ name?: string }>).map(t => t.name).filter(Boolean) as string[];
    ...
    const typedStructure = structure as {
      complexity?: string;
      isMonorepo?: boolean;
      packages?: unknown[];
