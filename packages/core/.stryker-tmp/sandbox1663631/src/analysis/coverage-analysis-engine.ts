...
export class CoverageAnalysisEngine {
  ...
  private async processCoverageData(
    ...
    if (!mergedCoverage) {
      throw new Error('No coverage data available for analysis');
    }
   ...
   * Merge coverage data from multiple tools
   */
  private mergeCoverageData(coverageResults: ToolResult[]): CoverageData | null {
    if (coverageResults.length === 1) {
      return coverageResults[0].metrics.coverage ?? null;
    }
