...
export class PerformanceOptimizer {
  ...
  detectBottlenecks(): Array<{type: string, impact: string, description: string}> {
    ...
    if (systemMetrics.memoryUsage && systemMetrics.memoryUsage.peak > 500 * 1024 * 1024) { // 500MB
      bottlenecks.push({
        type: 'memory',
        impact: 'high',
  ...
  updateConfig(newConfig: Partial<PerformanceOptimizerConfig>): void {
    ...
    if (newConfig.resourceManagement) {
      ...
      if (thresholds.memoryLimit <= 0 || (thresholds.cpuThreshold && thresholds.cpuThreshold > 100) || thresholds.throttlingThreshold <= 0) {
