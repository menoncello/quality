...
export class ScoringAlgorithm {
  ...
  generateInsights(
    ...
    if (coverage?.lines?.percentage && coverage.lines.percentage >= 90) {
    ...
    if (coverage?.lines?.percentage && coverage.lines.percentage < 60) {
  ...
  private calculateBonuses(
    ...
    // High coverage bonus
    if (coverage?.lines?.percentage && coverage.lines.percentage >= 95) {
