...
/**
 * Interface for detailed coverage data structure
 */
...
/**
 * Interface for file-level coverage data from coverage reports
 */
interface FileCoverageData {
  l?: Record<string, number>; // Line coverage
  f?: Record<string, unknown>; // Function coverage
  b?: Record<string, unknown>; // Branch coverage
  s?: Record<string, unknown>; // Statement coverage
}

/**
 * Interface for function coverage data
 */
interface _FunctionInfo {
  line: number;
  count: number;
}

/**
 * Interface for branch coverage data
 */
interface _BranchInfo {
  line: number;
  count: number;
}

/**
 * Type guard for function info arrays
 */
function isFunctionInfoArray(data: unknown): data is [number, number] {
  return Array.isArray(data) && data.length >= 2 && typeof data[0] === 'number' && typeof data[1] === 'number';
}

/**
 * Type guard for branch info arrays
 */
function isBranchInfoArray(data: unknown): data is [number, number] {
  return Array.isArray(data) && data.length >= 2 && typeof data[0] === 'number' && typeof data[1] === 'number';
}
...
export class CoverageAnalyzer {
  ...
  private async processFileCoverage(
    ...
    const data = detailedData as DetailedCoverageData;
    if (!data?.files) {
      return files;
    }
  ...
  private processSingleFile(
    ...
    const coverageData = fileData as FileCoverageData;
    const lines = this.processLineCoverage(coverageData.l ?? {});
    const functions = this.processFunctionCoverage(coverageData.f ?? {});
    const branches = this.processBranchCoverage(coverageData.b ?? {});
    const statements = this.processStatementCoverage(coverageData.s ?? {});
  ...
  private processFunctionCoverage(functionData: Record<string, unknown>): FunctionCoverage[] {
    ...
    for (const [funcName, funcInfo] of Object.entries(functionData)) {
      if (isFunctionInfoArray(funcInfo)) {
        const line: number = funcInfo[0];
        const count: number = funcInfo[1];
        functions.push({
          name: funcName,
          startLine: line,
  ...
  private processBranchCoverage(branchData: Record<string, unknown>): BranchCoverage[] {
    ...
    for (const [_branchKey, branchInfo] of Object.entries(branchData)) {
      if (isBranchInfoArray(branchInfo)) {
        const line: number = branchInfo[0];
        const count: number = branchInfo[1];
        branches.push({
  ...
  private calculateFunctionRiskScore(func: FunctionCoverage): number {
    return Math.min(func.complexity * 2 + (func.parameters ?? 0), 10);
  }

  private assessFunctionImpact(func: FunctionCoverage): 'low' | 'medium' | 'high' | 'critical' {
    return this.assessImpact(this.calculateFunctionRiskScore(func));
  }

  private generateFunctionTestRecommendation(func: FunctionCoverage): string {
    return `Write unit tests for function '${func.name}' covering all execution paths and edge cases.`;
  }

  private calculateFunctionPriority(func: FunctionCoverage): number {
    return this.calculateFunctionRiskScore(func) * 10;
  }

  private generateTestSuggestionsForFunction(func: FunctionCoverage): TestSuggestion[] {
    return [
      {
        type: 'unit' as const,
  ...
  private calculateBranchRiskScore(branch: BranchCoverage): number {
    return branch.isCritical ? 8 : 4;
  }

  private assessBranchImpact(branch: BranchCoverage): 'low' | 'medium' | 'high' | 'critical' {
    return this.assessImpact(this.calculateBranchRiskScore(branch));
  }

  private generateBranchTestRecommendation(branch: BranchCoverage): string {
    return `Add test to cover the uncovered branch at line ${branch.lineNumber}.`;
  }

  private calculateBranchPriority(branch: BranchCoverage): number {
    return this.calculateBranchRiskScore(branch) * 10;
  }

  private generateTestSuggestionsForBranch(branch: BranchCoverage): TestSuggestion[] {
    return [
      {
        type: 'unit' as const,
