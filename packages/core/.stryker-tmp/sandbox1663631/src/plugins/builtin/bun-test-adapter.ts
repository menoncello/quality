...
/**
 * Configuration interface for Bun Test adapter
 */
interface BunTestConfiguration extends Record<string, unknown> {
  testMatch: string[];
  ...
  enableAdvancedCoverage: boolean;
...
export class BunTestAdapter extends BaseToolAdapter {
  ...
  getDefaultConfig(): ToolConfiguration {
    const defaultConfig: BunTestConfiguration = {
  ...
  validateConfig(config: ToolConfiguration): ValidationResult {
    ...
    const cfg = config.config as BunTestConfiguration;
    ...
    if (cfg.coverageThreshold) {
      const thresholdKeys: (keyof CoverageThresholds)[] = ['statements', 'branches', 'functions', 'lines', 'criticalPaths'];

      for (const key of thresholdKeys) {
        const value = cfg.coverageThreshold[key as keyof typeof cfg.coverageThreshold];
...
Usage:
  ...
  protected async executeTool(context: AnalysisContext): Promise<ToolResult> {
    ...
    const testCommand = this.buildBunTestCommand(context, config as BunTestConfiguration);
    ...
    const toolConfig = this.getToolConfig() as BunTestConfiguration;
    if (toolConfig.coverage) {
      coverage = await this.collectCoverage(context, toolConfig);
    }
  ...
  private async executeIncremental(context: AnalysisContext): Promise<ToolResult> {
    ...
    const config = this.getToolConfig() as BunTestConfiguration;
    const relevantFiles = context.changedFiles.filter(file =>
      this.shouldProcessFile(file, config.testMatch, config.testPathIgnorePatterns)
    );
   ...
   */
  private buildBunTestCommand(context: AnalysisContext, config: BunTestConfiguration, files?: string[]): { cmd: string; args: string[] } {
    const cmd = 'bun';
   ...
   */
  private async collectCoverage(context: AnalysisContext, config: BunTestConfiguration): Promise<CoverageData | undefined> {
   ...
   */
  private getCoverageAnalyzerConfig(config: BunTestConfiguration): Partial<CoverageConfiguration> {
    return {
      thresholds: {
        overall: config.coverageThreshold.statements, // Use statements as overall for consistency
        ...
        criticalPaths: config.coverageThreshold.criticalPaths
      },
      criticalPaths: config.criticalPaths.map((path, index) => ({
        name: `critical-path-${index}`,
        patterns: [path],
        requiredCoverage: config.coverageThreshold.criticalPaths,
        description: `Critical path coverage requirement`
      })),
      exclusions: config.coverageExclusions,
      includePatterns: config.coverageIncludePatterns,
      enableTrending: config.enableTrending,
      enableQualityScoring: config.enableQualityScoring,
      enableRiskAssessment: config.enableRiskAssessment
    };
  }
  ...
  protected parseOutput(stdout: string, stderr: string, context: AnalysisContext): Issue[] {
    ...
    const toolConfig = context.config.tools?.find(t => t.name === 'bun-test')?.config as BunTestConfiguration;
    if (toolConfig?.coverageThreshold) {
      const coverageViolations = this.checkCoverageThresholds(output, toolConfig.coverageThreshold);
   ...
   */
  private checkCoverageThresholds(output: string, thresholds: CoverageThresholds): Issue[] {
    const issues: Issue[] = [];
