...
/**
 * Configuration interface for Prettier adapter
 */
interface PrettierConfiguration extends Record<string, unknown> {
  configFile: string;
  ignorePath: string;
  extensions: string[];
  check: boolean;
...
export class PrettierAdapter extends BaseToolAdapter {
  ...
  getDefaultConfig(): ToolConfiguration {
    const defaultConfig: PrettierConfiguration = {
  ...
  validateConfig(config: ToolConfiguration): ValidationResult {
    ...
    const cfg = config.config as PrettierConfiguration;
...
Usage:
  ...
  protected async executeTool(context: AnalysisContext): Promise<ToolResult> {
    ...
    const command = this.buildPrettierCommand(context, config as PrettierConfiguration);
  ...
  private async executeIncremental(context: AnalysisContext): Promise<ToolResult> {
    ...
    const config = this.getToolConfig() as PrettierConfiguration;
    const relevantFiles = context.changedFiles.filter(file =>
      this.shouldProcessFile(file, config.extensions)
    );
   ...
   */
  private buildPrettierCommand(context: AnalysisContext, config: PrettierConfiguration, files?: string[]): { cmd: string; args: string[] } {
