...
Usage:
   ...
   */
  private buildESLintCommand(context: AnalysisContext, config: Record<string, unknown>, files?: string[]): { cmd: string; args: string[] } {
    ...
    args.push('--format', (config.format as string) || 'json');

    if (config.maxWarnings !== undefined && config.maxWarnings !== null) {
    ...
    if (config.cache) {
      args.push('--cache');
      if (config.cacheLocation && typeof config.cacheLocation === 'string') {
