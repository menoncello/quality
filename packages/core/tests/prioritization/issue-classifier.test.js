...
describe('IssueClassifier', () => {
    ...
    describe('classifyIssue', () => {
        ...
        it('should handle errors gracefully when model is not ready', async () => {
            ...
            faultyClassifier.model = null;
            await expect(faultyClassifier.classifyIssue(mockIssue, mockContext)).rejects.toThrow('Classification model not loaded');
        });
    });
    describe('trainModel', () => {
        ...
        it('should reject insufficient training data', async () => {
            ...
            await expect(classifier.trainModel(insufficientData)).rejects.toThrow('Training data must contain at least 10 samples');
        });
        it('should validate training data structure', async () => {
            const invalidData = Array(15).fill(null).map((_, index) => ({
                issueId: index === 5 ? '' : `issue-${index}`, // One missing issueId
                features: index === 10 ? null : {
                    codeComplexity: 0.5,
                    changeFrequency: 0.5,
                    teamImpact: 0.5,
                    userFacingImpact: 0.5,
            ...
            await expect(classifier.trainModel(invalidData)).rejects.toThrow(/Invalid training data/);
        });
        it('should handle concurrent training attempts', async () => {
            const trainingData = Array(15).fill(null).map((_, index) => ({
                issueId: `issue-${index}`,
                features: {
                    codeComplexity: 0.5,
                    changeFrequency: 0.5,
            ...
            await expect(classifier.trainModel(trainingData)).rejects.toThrow('Model training already in progress');
    ...
    describe('classification validation', () => {
        it('should validate and correct invalid classification results', async () => {
            ...
            expect(['bug', 'performance', 'security', 'maintainability', 'documentation', 'feature']).toContain(classification.category);
        });
    });
